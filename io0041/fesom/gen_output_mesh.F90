...
#include "netcdf.inc" 
  ...
  !OR not needed for coupling, as we only initialize together with restart
  ...
  status = nf_def_var(ncid, 'cluster_area', NF_FLOAT, 2, dimids, area_varid)
  if (status .ne. nf_noerr) call handle_err(status)
