...
fesomMeltPath= '/work/ollie/orichter/MisomipPlus/io0015/fesomdata/io0015.1065.forcing.diag.nc';
fesomCoordPath= '/work/ollie/orichter/MisomipPlus/io0015/fesommesh/1065.30/nod2d.out';
...
indfes=transpose(nodes(4,:));

intNodFes = indfes==0;
xfes=xfes(intNodFes);
yfes=yfes(intNodFes);
wnetFes=wnetFes(intNodFes);
...
interp = scatteredInterpolant(xfes,yfes,wnetFes,'linear','none');
wnetUa = interp(xUa,yUa);
idxNaN = isnan(wnetUa);
idx = knnsearch([xfes yfes],[xUa(idxNaN) yUa(idxNaN)],'K',3);
wnetUa(idxNaN) = mean(wnetFes(idx),2);
