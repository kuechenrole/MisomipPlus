...
if contains(plots,'-save-')
    ...
    if strcmp(CtrlVar.DefineOutputsInfostring,'Last call')==1
        ...
        %FileName=sprintf('%s/%.2f-Nodes%i-Ele%i-Tri%i-kH%i-%s.mat',...
        %    CtrlVar.Outputsdirectory,time,MUA.Nnodes,MUA.Nele,MUA.nod,1000*CtrlVar.kH,CtrlVar.Experiment);
        FileName=sprintf('%s/%s-Nodes%i-Ele%i-Tri%i-kH%i-%s.mat',...
            CtrlVar.Outputsdirectory,UserVar.FinalOutputID,MUA.Nnodes,MUA.Nele,MUA.nod,1000*CtrlVar.kH,CtrlVar.Experiment);
    ...
    elseif strcmp(CtrlVar.DefineOutputsInfostring,'First call')==1
        ...
        %    CtrlVar.Outputsdirectory,time,MUA.Nnodes,MUA.Nele,MUA.nod,1000*CtrlVar.kH,CtrlVar.Experiment);
        FileName=sprintf('%s/%s-Nodes%i-Ele%i-Tri%i-kH%i-%s.mat',...
            CtrlVar.Outputsdirectory,UserVar.StartOutputID,MUA.Nnodes,MUA.Nele,MUA.nod,1000*CtrlVar.kH,CtrlVar.Experiment);
        fprintf(' Saving data in %s \n',FileName)
        save(FileName,'CtrlVar','MUA','F','BCs','RunInfo')
