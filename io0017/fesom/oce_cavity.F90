...
subroutine cavity_heat_water_fluxes_3eq
  ...
  real (kind=8)  :: temp,sal,tin,zice,fwfo,fwfn,hfo,hfn
...
!RT RG4190-
     ...
     fwfo=-water_flux(n)
     hfo=-heat_flux(n)
     fwfn=min(fwfo,50/(365.25*24*3600)) !limiting melt to 50 m/yr
     hfn=hfo*(fwfn/fwfo)
     net_heat_flux(n)=hfn   ! positive down
     fresh_wa_flux(n)=fwfn
