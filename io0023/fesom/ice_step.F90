...
!#ifdef use_cavity     !LK89140
!  call clean_cavity_m !!! we don't need this if there is no flux of tracers to the cavity nodes
!#endif
  t2=MPI_Wtime()   
...
subroutine clean_cavity_vel
  ...
  do row=1,myDim_nod2d+eDim_nod2d           
     if(cavity_flag_extended(row)==1) then  !LK89140
        u_ice(row)=0.
        v_ice(row)=0.
     endif
  enddo
