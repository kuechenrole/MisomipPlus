...
subroutine cavity_heat_water_fluxes_3eq
  ...
  integer :: lev,nodes(3)
  logical :: mask(3)
  real(kind=8) :: uf_tmp(size(uf)),sub

  uf_tmp = uf
  do n = 1,myDim_elem2d
     if(any(cavity_flag_nod2d(elem2d_nodes(:,n))==0)) cycle
     mask = index_nod2D(elem2d_nodes(:,n))==1
     if(any(mask)) then
       nodes = elem2d_nodes(:,n)
       do lev = 1,2
         uf_tmp(nod3d_below_nod2d(lev,pack(nodes,mask)))=sum(uf(nod3d_below_nod2d(lev,pack(nodes,mask==.false.))))/count(mask==.false.)
       enddo
     endif
  enddo
...
!!RT test-
     ...
     vt1  = sqrt(((uf_tmp(nk)+uf_tmp(nk2))*0.5)**2+((uf_tmp(nk+n3)+uf_tmp(nk2+n3))*0.5)**2)
