! Sea ice advection routines
! The fct advection scheme is in a separate file.

subroutine ice_matrices_setup
  ! ice matrices setting up
...
#ifdef use_cavity
     if (any(cavity_flag_nod2d(elnodes)==1)) cycle !LK89140
#endif
...
subroutine icestiff_fill_gls
  ...
  ! Coded by Sergey Danilov
  ! Reviewed by Qiang Wang
  ! Modified by Qiang Wang to allow for out-going open boundary application.
  !--------------------------------------------------------------
...
subroutine ice_solve
  ...
  do row=1,myDim_nod2D 
#ifdef use_cavity   !LK89140 0/0 
     if(cavity_flag_nod2d(row)==1 .or. ice_lump(row)==0.) cycle
#endif 
  ...
  do n=1,num_iter_solve_ice-1
     do row=1,myDim_nod2D
#ifdef use_cavity
        if(cavity_flag_nod2d(row)==1 .or. ice_lump(row)==0.) cycle !LK89140 0/0 
#endif                
...
#ifdef use_cavity
        if(cavity_flag_nod2d(row)==1 .or. ice_lump(row)==0.) cycle !LK89140  
#endif             
