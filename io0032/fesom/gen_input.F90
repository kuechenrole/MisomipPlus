...
#include "netcdf.inc" 
  ...
  integer                   :: status, ncid, j, dimid_rec, nrec, n
...
! OR: remapping in the coupled run can produce velocities at the bry. here we
! set them to zero
#ifdef isomip
  !write (*,*) 'setting bry velocities to zero for coupled run'
  do n=1,myDim_nod3D+eDim_nod3D
    if ((index_nod3D(n) == 11) .or. (index_nod3D(n) == 21) .or. (index_nod3D(n) == 31)) then
       uf(n)=0.0
       uf(n+n3)=0.0
    endif
  enddo
