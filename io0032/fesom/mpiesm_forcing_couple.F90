subroutine init_atm_forcing
  ...
  ! assume forcing data from mpi-esm
  use o_param
  use o_mesh
  use o_array
  use i_therm_parms
...
!  use g_read_other_NetCDF
  ...
  implicit none
  !
  integer, parameter        		:: nci=192, ncj=96     ! mpi-esm grid
...
!rt  real(kind=8), allocatable             :: aux(:) 
  ...
  if (wind_data_source(1:7).ne.'mpi-esm') then
   write(*,*)'STOP: This subroutine is designed only for use of mpi-esm data.'
   stop
  endif

  if (wind_data_source.eq.'mpi-esm_hist') then
   ! mpi-esm_hist data start 01.01.1950
   itime=(yearnew-1950)*365 + int(float(yearnew-1949)/4.) + daynew ! considering leap years
  elseif (wind_data_source.eq.'mpi-esm_rcp85' .or. wind_data_source.eq.'mpi-esm_rcp45') then
   itime=(yearnew-2006)*365 + int(float(yearnew-2005)/4.) + daynew ! mspi-esm RCP data start 01.01.2006
  else
 ...
 if (wind_data_source.eq.'mpi-esm_hist') then
  if (itime>20454) then
   write(*,*)'limit of itime is reached, itime=',itime
   stop
  endif
 elseif (wind_data_source(1:11).eq.'mpi-esm_rcp') then
  if (itime>34698) then
   write(*,*)'limit of itime is reached, itime=',itime
   stop
  endif
 else
  write(*,*)'STOP: please define what to do in this case: label 2'
  stop
 endif
  ...
  do i=1,2
     ...
     write(*,*)'init i,itime=',i,itime

#include "read_mpi-esm_data.incF90"
  ...
  !==========================================================================
  ! runoff
  ! is not provided for the HadCM3, HadGem2, and mpi-esm experiments
...
if(update_forcing_flag(wind_ttp_ind)==1) then
 ...
 if (wind_data_source.eq.'mpi-esm_hist') then
  if (itime>20454) then
   write(*,*)'limit itime to 20454'
   itime=20454  ! HadCM3 files contain 56 years: very last record is used twice
  endif
 elseif (wind_data_source(1:11).eq.'mpi-esm_rcp') then
  if (itime>34698) then
   write(*,*)'limit itime to 34698'
   itime=34698  ! HadCM3 files contain 95 years: very last record is used twice
  endif
 else
  write(*,*)'STOP: please define what to do in this case: label 2'
  stop
...
!---------------------------------------------------------------------------------------------------
!
subroutine rotate_mpi_esm_wind(xarray, yarray)
  ! rotate wind on mpi-esm grid from geographical coord. to rotated coordinates.
  use o_param
  use g_rotate_grid
  implicit none

  integer, parameter 	:: ni=192, nj=96  ! mpi-esm grid
  integer               :: i, j
...
!mpi-esm latitudes:
cy=(/ -88.5721664428711, -86.7225341796875, -84.8619689941406,  &
    -82.9989395141602, -81.1349792480469, -79.2705612182617,  &
    -77.405891418457, -75.5410614013672, -73.6761322021484,  &
    -71.8111343383789, -69.9460830688477, -68.0809936523438,  &
    -66.2158737182617, -64.3507308959961, -62.4855690002441,  &
    -60.6203956604004, -58.7552108764648, -56.8900146484375,  &
    -55.0248069763184, -53.159595489502, -51.294376373291, -49.4291534423828,  &
    -47.5639266967773, -45.6986923217773, -43.8334579467773,  &
    -41.9682197570801, -40.1029777526855, -38.237735748291,  &
    -36.3724899291992, -34.5072441101074, -32.6419944763184,  &
    -30.7767448425293, -28.9114933013916, -27.0462398529053,  &
    -25.1809864044189, -23.315731048584, -21.450475692749, -19.5852184295654,  &
    -17.7199611663818, -15.8547039031982, -13.9894456863403,  &
    -12.1241874694824, -10.2589282989502, -8.39366912841797,  &
    -6.52840948104858, -4.6631498336792, -2.79788970947266,  &
    -0.932630002498627, 0.932629883289337, 2.79788994789124, 4.6631498336792,  &
    6.52840948104858, 8.39366912841797, 10.2589282989502, 12.1241874694824,  &
    13.9894456863403, 15.8547039031982, 17.7199611663818, 19.5852184295654,  &
    21.450475692749, 23.315731048584, 25.1809864044189, 27.0462398529053,  &
    28.9114933013916, 30.7767448425293, 32.6419944763184, 34.5072441101074,  &
    36.3724899291992, 38.237735748291, 40.1029777526855, 41.9682197570801,  &
    43.8334579467773, 45.6986923217773, 47.5639266967773, 49.4291534423828,  &
    51.294376373291, 53.159595489502, 55.0248069763184, 56.8900108337402,  &
    58.7552108764648, 60.6203956604004, 62.4855690002441, 64.3507308959961,  &
    66.2158737182617, 68.0809936523438, 69.9460830688477, 71.8111343383789,  &
    73.6761322021484, 75.5410614013672, 77.405891418457, 79.2705612182617,  &
    81.1349792480469, 82.9989395141602, 84.8619689941406, 86.7225341796875,  &
    88.5721664428711 /) * rad
...
!mpi-esm longitudes
cx=(/0., 1.875, 3.75, 5.625, 7.5, 9.375, 11.25, 13.125, 15., 16.875, 18.75, &
    ...
    348.75, 350.625, 352.5, 354.375, 356.25, 358.125 /)*rad
...
end subroutine rotate_mpi_esm_wind
!
!---------------------------------------------------------------------------------------------------
