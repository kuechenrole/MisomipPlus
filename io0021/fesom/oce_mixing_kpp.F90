...
! the limits are: umin =< ustar(i) <=umax ; Bo(i)>=zmin/(vonk*0.1*abs(z))
    ...
    enddo
    if (limit_forcing) then
       call MPI_AllREDUCE(nlims_bf, gnlims_bf, 1, MPI_INTEGER, MPI_SUM, MPI_COMM_WORLD, MPIerr)
       if (gnlims_bf>0) then
...
subroutine is_communicated2(arr)
  ...
  aux=arr
  call com_2D(aux)
  call MPI_AllREDUCE(minval(aux-arr), gmin, 1, MPI_DOUBLE_PRECISION, MPI_MIN, MPI_COMM_WORLD, MPIerr)
  call MPI_AllREDUCE(maxval(aux-arr), gmax, 1, MPI_DOUBLE_PRECISION, MPI_MAX, MPI_COMM_WORLD, MPIerr)
